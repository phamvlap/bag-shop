<?php 
	...
	$filters['date'] = isset($_GET['filter-invoice-date']) ? $_GET['filter-invoice-date'] : false;
	$filters['total'] = isset($_GET['filter-invoice-total']) ? $_GET['filter-invoice-total'] : false;
	$filters['status'] = isset($_GET['filter-invoice-status']) ? $_GET['filter-invoice-status'] : false;
?>
	...
	<div class="col col-md-3">
		<div class="input-group">
			...
			<select class="form-select" id="filter-invoice-date" name="filter-invoice-date">
				...
				<option 
					value="desc"
					<?php if(isset($filters['date']) && $filters['date'] === 'desc') echo 'selected'; else echo '';
					?>
				>Mới nhất</option>
				<option 
					value="asc"
					<?php if(isset($filters['date']) && $filters['date'] === 'asc') echo 'selected'; else echo '';
					?>
				>Cũ nhất</option>
			</select>
		</div>
	...
	<div class="col col-md-3">
		<div class="input-group">
			...
			<select class="form-select" id="filter-invoice-status" name="filter-invoice-status">
				...
				<option 
					value="0"
					<?php if(isset($filters['status']) && $filters['status'] !== false && (int)$filters['status'] === 0) echo 'selected'; else echo '';
					?>
				>Đang chờ duyệt</option>
				<option 
					value="1"
					<?php if(isset($filters['status']) && $filters['status'] !== false && (int)$filters['status'] === 1) echo 'selected'; else echo '';
					?>
